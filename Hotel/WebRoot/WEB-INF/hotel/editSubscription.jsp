...
<html>
  <head>  
    <title>My JSP 'editSubscription.jsp' starting page</title>
...
//  	提交校验
  ...
  <div id="content">
    ...
    <div class="subcontent">
      <form action="${pageContext.request.contextPath }/hotel/editSubscription03.do" onsubmit="return checkAll();">
        <fieldset>
          <legend>联系人信息</legend>
          <c:forEach items="${subscriptions02}" var="subscription">         	
          <div>
          	<input type="hidden" name="subscription.id" id="id" value="${subscription.id}"/>
            <label for="name">联系人</label>
            <input type="text" name="subscription.linkman" id="name" value="${subscription.linkman}" onblur="checkName();" onfocus="onf('name');"/><span id="nameMessage"></span>
          </div>
          <div>
            <label for="phone">联系电话</label>
            <input type="text" name="subscription.phone" id="phone" value="${subscription.phone}" onblur="checkPhone();" onfocus="onf('phone');"/><span id="phoneMessage"></span>
          </div>
          <div>
            <label for="email">电子邮件</label>
            <input type="text" name="subscription.email" id="email" value="${subscription.email}" onblur="checkEmai();" onfocus="onf('email');"/><span id="emailMessage"></span>
          </div>
          </c:forEach>
        </fieldset>
        <fieldset class="buttons">
          <input type="button" value="取消" onclick="location='${pageContext.request.contextPath }/hotel/memberCenter.do'"/>
